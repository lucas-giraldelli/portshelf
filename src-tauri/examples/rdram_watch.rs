...
//! cargo run --example rdram_watch -- <program> [log file]
//!
...
fn main() {
    ...
    let mut child = Command::new(&program).current_dir(program.parent().unwrap()).spawn().expect("start the port");
    ...
    loop {
        ...
        let snapshot = (|| -> std::io::Result<_> {
            ...
            let flags = ram.bytes(JIGGY_FLAGS, 13)?;
        ...
        match snapshot {
            Ok(now) if now != last => {
                ...
                writeln!(
                    log,
                    "[{:>6.1}s] osMemSize={mem_size:#x} notes={} jiggies_held={} jiggies_total={} mumbo_total={} collected={collected:?}",
